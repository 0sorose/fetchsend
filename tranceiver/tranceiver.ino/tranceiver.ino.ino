...
if (role == 1)  {
    ...
    char payload = remote.read();                            // Take the time, and send it.  This will block until complete
    Serial.print(payload); 
     if (!radio.write( &payload, sizeof(byte) )){
  ...
  if ( role == 0 )
  {
    ...
    if( radio.available()){
      ...
      while (radio.available()) {                                   // While there is data ready
        radio.read( &payload, sizeof(byte) );             // Get the payload
      //disp = payload;
      Serial.print(payload);
      }
